...
class MultiAgentEnv(gym.Env):
        ...
        if mode == 'human':
            ...
            for agent in self.world.agents:
                ...
                for other in self.world.agents:
                    ...
                    message2 += (agent.name + ' ' + str(agent.size))
            #print(message2)
        ...
        if True:
            ...
            for entity in self.world.entities:
                ...
                if 'agent' in entity.name:
                    ...
                    geom = rendering.make_polygon(entity.shape)# out line the border, which shape is square
                    #print(entity.shape)
    ...
    def inc_size(self):
        # 可以在这里改变大小
        for agent in self.agents:
            agent.size = agent.size * 1.01 if agent.adversary else agent.size
    ...
    def de_size(self):
        # 可以在这里改变大小
        for agent in self.agents:
            agent.size = agent.size - 50 * 0.005 if agent.adversary else agent.size
