...
class MADDPG:
    #改动 def __init__(self, obs_dim_list, act_dim_list, capacity, actor_lr, critic_lr, res_dir=None, device=None):
    def __init__(self, obs_dim_list, act_dim_list, capacity, actor_lr, critic_lr, res_dir=None, device=None, layer_id=None):
        """
        ...
        for obs_dim, act_dim in zip(obs_dim_list, act_dim_list):
            ...
            self.agents.append(Agent(obs_dim, act_dim, global_obs_dim, actor_lr, critic_lr, self.device, layer_id))
    ...
    #我认为多层次交互需要在这里执行,实际上添加了layer
    def select_action(self, obs):
        actions = []
        for n, agent in enumerate(self.agents):  # each agent select action according to their obs
            #agent.layer_id
#            if agent.layer_id[0] == 2:#两个导弹之间可以互相通信,关键是设置的时候应该也是[x,1]这种后面全是1，只用看第一个
